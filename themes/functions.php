...
<a href="http://validator.w3.org/checklink?uri={$fl->request->current_url}">links</a>
<a href="http://qa-dev.w3.org/i18n-checker/index?async=false&amp;docAddr={$fl->request->current_url}">i18n</a>
...
function login_menu() {
  $fl = CFloyd::Instance();
  if($fl->user['isAuthenticated']) {
    $items = "<a href='" . create_url('user/profile') . "'><img class='gravatar' src='" . get_gravatar(20) . "' alt=''> " . $fl->user['acronym'] . "</a> ";
    if($fl->user['hasRoleAdministrator']) {
 ...
 */
  ...
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CFloyd::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
  }
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CFloyd::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function theme_url($url) {
  return create_url(CFloyd::Instance()->themeUrl . "/{$url}");
}
...
function theme_parent_url($url) {
  return create_url(CFloyd::Instance()->themeParentUrl . "/{$url}");
}
...
function render_views($region='default') {
  return CFloyd::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CFloyd::Instance()->views->RegionHasView(func_get_args());
}
